...
def finite_row_mask(*arrays):
    ...
    for arr in arrays:
        ...
        if len(arr_np) != n:
            raise ValueError("All arrays must have the same row count for finite-row filtering.")
        if arr_np.ndim == 1:
            mask &= np.isfinite(arr_np)
        else:
            mask &= np.isfinite(arr_np).all(axis=1)
    return mask


def filter_rows_with_mask(mask, *arrays):
...
def train_autoencoder(
    ...
    if skipped_train_batches > 0 or skipped_val_batches > 0:
        print(
            f"[AE] Skipped non-finite batches: train={skipped_train_batches}, val={skipped_val_batches}"
        )
...
def train_classifier(model, train_loader, val_loader, epochs=200, lr=1e-3, patience=20):
    ...
    if skipped_train_batches > 0 or skipped_val_batches > 0:
        print(
            f"[CLS] Skipped non-finite batches: train={skipped_train_batches}, val={skipped_val_batches}"
        )
...
def main():
    ...
    finite_bread_mask = finite_row_mask(X_bread, chem_targets, temp_targets)
    removed_bread = int((~finite_bread_mask).sum())
    if removed_bread > 0:
        print(f"Dropping {removed_bread} non-finite rows from bread slice before AE/classifier training.")
        bread_df = bread_df.loc[finite_bread_mask].reset_index(drop=True)
        X_bread, chem_targets, temp_targets = filter_rows_with_mask(
            finite_bread_mask, X_bread, chem_targets, temp_targets
        )
    ...
    temp_norm = (temp_target / float(args.temperature_scale)).astype(np.float32)
    X_norm[:, temp_idx] = temp_norm[:, 0]
    ...
    finite_bread_cls_mask = finite_row_mask(X_bread_cls)
    if (~finite_bread_cls_mask).any():
        removed = int((~finite_bread_cls_mask).sum())
        print(f"Dropping {removed} non-finite rows from bread classifier set.")
        X_bread_cls, y_bread_cls = filter_rows_with_mask(finite_bread_cls_mask, X_bread_cls, y_bread_cls)

    finite_ham_cls_mask = finite_row_mask(X_ham_cls)
    if (~finite_ham_cls_mask).any():
        removed = int((~finite_ham_cls_mask).sum())
        print(f"Dropping {removed} non-finite rows from ham classifier set.")
        X_ham_cls, y_ham_cls = filter_rows_with_mask(finite_ham_cls_mask, X_ham_cls, y_ham_cls)
    ...
    finite_ham_chem_mask = finite_row_mask(X_ham_chem)
    if (~finite_ham_chem_mask).any():
        removed = int((~finite_ham_chem_mask).sum())
        print(f"Dropping {removed} non-finite rows from ham chem subset.")
        X_ham_chem, y_ham_chem = filter_rows_with_mask(finite_ham_chem_mask, X_ham_chem, y_ham_chem)

    finite_ham_rest_mask = finite_row_mask(X_ham_rest)
    if (~finite_ham_rest_mask).any():
        removed = int((~finite_ham_rest_mask).sum())
        print(f"Dropping {removed} non-finite rows from ham rest subset.")
        X_ham_rest, y_ham_rest = filter_rows_with_mask(finite_ham_rest_mask, X_ham_rest, y_ham_rest)
