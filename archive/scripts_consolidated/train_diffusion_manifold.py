#!/usr/bin/env python3
"""
DIFFUSION WITH MANIFOLD PRESERVATION
Standard diffusion learns to denoise but doesn't preserve multi-modal structure.
Add discriminator loss to force samples onto the actual data manifold (the "islands").
"""
...
class Discriminator(nn.Module):
    """Discriminator to push samples onto data manifold"""
...
print("\n" + "="*80)
print("TRAINING DIFFUSION WITH DISCRIMINATOR (MANIFOLD PRESERVATION)")
print("="*80)
...
opt_diff = torch.optim.AdamW(diffusion.parameters(), lr=LEARNING_RATE, weight_decay=0.01)
opt_disc = torch.optim.AdamW(discriminator.parameters(), lr=LEARNING_RATE * 0.5, weight_decay=0.01)

scheduler_diff = torch.optim.lr_scheduler.CosineAnnealingLR(opt_diff, MAX_EPOCHS)
scheduler_disc = torch.optim.lr_scheduler.CosineAnnealingLR(opt_disc, MAX_EPOCHS)
...
for epoch in range(MAX_EPOCHS):
    ...
    for (batch_latent,) in train_loader:
        ...
        # Real samples
        real_pred = discriminator(batch_latent)
        loss_real = nn.functional.binary_cross_entropy_with_logits(
            real_pred, torch.ones_like(real_pred)
        )
        
        # Fake samples (full diffusion sampling - expensive but accurate)
        with torch.no_grad():
            fake_samples = torch.FloatTensor(sample_ddpm(diffusion, n_samples=batch_size)).to(device)
        ...
        x_noisy, noise = forward_diffusion(batch_latent, t, diffusion)
        predicted_noise = diffusion(x_noisy, t)
        loss_denoise = nn.functional.mse_loss(predicted_noise, noise)
        
        # Adversarial loss (make discriminator think samples are real)
        # Sample at fewer timesteps for efficiency
        if (n_batches % 5) == 0:  # Every 5 batches
            fake_samples = torch.FloatTensor(sample_ddpm(diffusion, n_samples=batch_size)).to(device)
            fake_pred = discriminator(fake_samples)
            ...
            # Combined loss: denoising + adversarial (weighted)
            loss_total = loss_denoise + 0.1 * loss_adv  # 0.1 weight on adversarial
        ...
        epoch_loss_diff += loss_denoise.item()
    ...
    avg_loss_diff = epoch_loss_diff / n_batches
    avg_loss_disc = epoch_loss_disc / n_batches
    avg_loss_adv = epoch_loss_adv / max(1, n_batches // 5)
    
    if (epoch + 1) % 20 == 0:
        samples_norm = sample_ddpm(diffusion, n_samples=500)
        mean_dist, std, score = evaluate_samples(samples_norm)
        
        print(f"Epoch {epoch+1}: diff={avg_loss_diff:.4f}, disc={avg_loss_disc:.4f}, adv={avg_loss_adv:.4f} | "
        ...
        if abs(score - 1.0) < abs(best_score - 1.0):
            ...
            torch.save({
                ...
                'data_mean': DATA_MEAN,
                'data_std': DATA_STD
            }, 'models/diffusion_manifold_best.pt')

# Final evaluation
final_samples_norm = sample_ddpm(diffusion, n_samples=500)
...
axes[0].scatter(all_pca[500:, 0], all_pca[500:, 1], alpha=0.5, s=30, label='Diffusion+Disc', color='red')
axes[0].set_title(f'PCA: Manifold-Aware Diffusion (DEB)\nScore: {final_score:.3f}', fontweight='bold')
...
axes[1].hist(final_samples.flatten(), bins=50, alpha=0.5, label='Diffusion+Disc', density=True, color='red')
...
plt.tight_layout()
plt.savefig('images/diffusion_manifold_single_chemical.png', dpi=300, bbox_inches='tight')
print(f"\n✓ Saved: images/diffusion_manifold_single_chemical.png")

if final_score > 0.8 and final_score < 1.2:
    print("\n✓ SUCCESS! Manifold-aware diffusion preserves data structure!")
else:
    print(f"\n⚠ Score {final_score:.3f} - discriminator may need more tuning")
